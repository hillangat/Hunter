...
public abstract class AbstractCalculator implements Calculator {
	...
	public static double basicMultiplyTill(double base, double multiplier, double increment) {
		...
		if (increment < 0)
			throw new IllegalArgumentException("No passing a negative increment. Increment = " + increment);
		double result = 1;
	...
	public static double[] basicMultiplyTillWithChange(double base, double multiplier, double increment,
			double percentage) {
	...
	public static double handleRaiseToDecimal(double base, double decimal) {
		...
		if (decimal > 1)
			throw new IllegalArgumentException(
					"handleRaiseToDecimal(double base, double decimal) accepts only decimal or  0 or 1 exponents! input = "
							+ decimal);
	...
	public static double advancedRaisedTo(double number, double root) {
		...
		if (hasDecimal) {
			...
			final double limitHolder = decDivider;
			decDivider = 1;
		...
		sqrt = isNormalize ? sqrt * square : sqrt;
		log.info(" isNormalized = " + isNormalize + " Sqrt = " + sqrt);
		sqrt = hasDecimal && !isNormalize ? sqrt / decDivider : sqrt;
		log.info(" hasDecimal = " + hasDecimal + " Sqrt = " + sqrt + " decDivider = " + decDivider);
	...
	public static void main(String[] args) {
		/*
		 * double [] result = basicMultiplyTillWithChange(1000, 2, 0.001, 98); double
		 * raisTo = basicRaiseTo(result[0], result[1]);
		 * System.out.println(basicMultiplyTillWithChange(1000, 2, 0.001, 98));
		 * System.out.println("RaisTo >> " + raisTo);
		 */
		String themeList = "0,78,87;68,222,254;0,65,64&21,67,0;16,254,143;0,135,46";
		String[] themes = themeList.split("&");
		for (String t : themes) {
			String[] colorsArr = t.split(";");
			String[] top = colorsArr[0].split(","), middle = colorsArr[1].split(","), bottom = colorsArr[2].split(",");
			prnt(getColor(top), getColor(middle), getColor(bottom));
		}
	}

	private static void printTheme(String[] theme) {
		System.out.println("..............");
	...
	private static void prnt(String r, String g, String b) {
		String c = "{\r\n" + " \"name\": \"\",\r\n" + " \"default\": false,\r\n" + " \"top\": ::1::,\r\n"
				+ " \"middle\": ::2::,\r\n" + " \"bottom\": ::3::\r\n" + "},";
		c = c.replace("::1::", r);
		c = c.replace("::2::", g);
